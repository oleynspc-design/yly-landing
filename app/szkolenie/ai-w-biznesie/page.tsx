...
export default function AiWBiznesPage() {
  ...
  const renderBlock = (block: ContentBlock, index: number) => {
    switch (block.type) {
      ...
      case "highlight": {
        ...
        return (
          <div key={index} className="my-8 p-6 rounded-2xl bg-emerald-600/10 border border-emerald-500/20 relative overflow-hidden">
          ...
          <ul key={index} className="space-y-4 mb-8">
            {(block.items || []).map((item: string, i: number) => (
              <li key={i} className="flex items-start gap-3 bg-[#111] p-4 rounded-xl border border-white/5">
                <ChevronRight className="text-emerald-500 flex-shrink-0 mt-1" size={18} />
      ...
      case "prompt-box":
        return (
          <div key={index} className="my-8 rounded-2xl border border-white/10 bg-[#0c0c0c] overflow-hidden">
            ...
            <div className="p-6">
              <pre className="text-sm font-mono text-emerald-100 whitespace-pre-wrap leading-relaxed">{block.code}</pre>
            </div>
          </div>
        );
      default:
        ...
        <div className="max-w-5xl mx-auto px-4 py-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h1 className="text-4xl font-black text-white">AI w Biznesie</h1>
                <p className="text-gray-500 text-sm mt-1">Zastosowania komercyjne AI · 4 Moduły · 12 Lekcji · Egzamin {quizQuestions.length} pytań</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-3 mt-6 mb-12">
              <span className="px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium">12 lekcji</span>
            ...
            {completedLessons.length > 0 && (
              <div className="mb-10 p-4 rounded-xl bg-[#0f0f0f] border border-white/5">
                ...
                <div className="w-full h-2 bg-[#1a1a1a] rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-emerald-600 to-teal-500 rounded-full transition-all" style={{ width: `${(completedLessons.length / lessons.length) * 100}%` }} />
                </div>
              </div>
            )}
          </motion.div>
          ...
          {semesters.map((sem, si) => {
            ...
            <motion.div key={sem.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: si * 0.1 }} className="mb-10 relative">
              <div className="flex items-center gap-3 mb-4">
                <Layers size={20} className={semLocked ? "text-gray-600" : "text-emerald-400"} />
                <h2 className={`text-xl font-bold ${semLocked ? "text-gray-600" : "text-white"}`}>{sem.name}</h2>
                {semLocked && <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold">PREMIUM</span>}
              </div>
              {semLocked ? (
                <div className="relative">
                      ...
                      return (
                        <div key={li} className="w-full text-left p-5 rounded-xl border bg-[#0f0f0f] border-white/5">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-emerald-600/20">
                              <span className="text-emerald-400 font-bold text-sm">{li + 1}</span>
                            </div>
                            <div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center p-6 rounded-2xl bg-[#0a0a0a]/90 border border-emerald-500/20 backdrop-blur-sm max-w-sm">
                      ...
                      <a href="/sklep" className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold transition-all">
                        Odblokuj dost\u0119p
                      </a>
                    </div>
                  </div>
              ...
              <div className="space-y-3">
                {sem.lessons.map((li) => {
                  ...
                  if (!lesson) return null;
                  return (
                    <button key={li} onClick={() => startLesson(li)} className={`w-full text-left p-5 rounded-xl border transition-all group ${done ? "bg-green-950/10 border-green-500/20 hover:border-green-500/40" : "bg-[#0f0f0f] border-white/5 hover:border-emerald-500/30"}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${done ? "bg-green-500/20" : "bg-emerald-600/20"}`}>
                            {done ? <CheckCircle size={20} className="text-green-400" /> : <span className="text-emerald-400 font-bold text-sm">{li + 1}</span>}
                          </div>
                          <div>
                            <h3 className="text-white font-semibold group-hover:text-emerald-400 transition-colors">{lesson.title}</h3>
                            <span className="text-gray-500 text-xs">{lesson.duration}</span>
                          </div>
                        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-12">
            <button
              ...
              className={`w-full p-6 rounded-2xl border text-left transition-all ${completedLessons.length >= lessons.length ? "bg-gradient-to-r from-emerald-900/20 to-teal-900/20 border-emerald-500/30 hover:border-emerald-500/50 cursor-pointer" : "bg-[#0a0a0a] border-white/5 opacity-50 cursor-not-allowed"}`}
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm">
                    {completedLessons.length >= lessons.length ? "Ukończyłeś wszystkie lekcje. Rozpocznij egzamin!" : `Ukończ wszystkie ${lessons.length} lekcji (${completedLessons.length}/${lessons.length})`}
                  </p>
                </div>
              </div>
            </button>
            ...
            {!quizSubmitted && (
              <>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h1 className="text-3xl font-black text-white">Egzamin: AI w Biznesie</h1>
                </div>
                ...
                <div className="flex gap-1 mb-8">
                  {Array.from({ length: totalQuizPages }).map((_, i) => (
                    <button key={i} onClick={() => setQuizPage(i)} className={`h-2 rounded-full transition-all ${i === quizPage ? "w-8 bg-emerald-500" : "w-4 bg-white/10 hover:bg-white/20"}`} />
                  ))}
                </div>
              </>
            )}
              ...
              <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="text-center">
                ...
                <div className="text-xl font-medium mb-10">
                  {calculateScore() >= PASS_SCORE
                    ? <span className="text-green-400">Gratulacje! Certyfikat AI w Biznesie odblokowany! 🏆</span>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  {calculateScore() < PASS_SCORE && (
                    <button onClick={() => { setQuizAnswers([]); setQuizSubmitted(false); setQuizPage(0); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="px-8 py-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold transition-all">
                      Podejdź ponownie
                    </button>
                  )}
                </div>
              ...
              <>
                  ...
                  {pageQuestions.map((q, localIdx) => {
                    ...
                    return (
                      <div key={qi} className="p-6 rounded-2xl bg-[#0f0f0f] border border-white/5">
                        <h3 className="text-lg font-semibold text-white mb-4"><span className="text-emerald-500 font-black mr-2">{qi + 1}.</span>{q.question}</h3>
                        <div className="space-y-2">
                          {q.options.map((opt, oi) => {
                            const sel = quizAnswers[qi] === oi;
                            return (
                              <button key={oi} onClick={() => handleAnswer(qi, oi)} className={`w-full text-left p-3.5 rounded-xl border transition-all ${sel ? "bg-emerald-600/20 border-emerald-500/50 text-white" : "bg-[#151515] border-white/5 text-gray-300 hover:border-white/10"}`}>
                                <div className="flex items-start gap-3">
                                  <span className={`w-5 h-5 rounded-full border flex items-center justify-center text-xs font-medium mt-0.5 flex-shrink-0 ${sel ? "border-emerald-500 bg-emerald-500 text-white" : "border-gray-600 text-gray-500"}`}>
                ...
                <div className="flex items-center justify-between mt-8 gap-4">
                  ...
                  {quizPage < totalQuizPages - 1 ? (
                    <button onClick={() => { setQuizPage(quizPage + 1); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-medium transition-all">
                      Następna<ChevronRight size={16} className="inline ml-2" />
                    </button>
                  ) : (
                    <button onClick={handleSubmitQuiz} disabled={quizAnswers.filter(a => a !== undefined).length !== quizQuestions.length} className="px-8 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-800 disabled:text-gray-500 text-white font-bold transition-all">
  ...
  return (
    <main className="min-h-screen bg-[#080808]">
      <div className="fixed top-0 left-0 w-full h-1.5 bg-[#111] z-50">
        <motion.div className="h-full bg-gradient-to-r from-emerald-600 to-teal-500" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.5 }} />
      </div>
      <div className="max-w-4xl mx-auto px-4 py-16" ref={contentRef}>
        ...
        {activeMicroQuiz ? (
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-white/10">
            ...
            <button onClick={handleNext} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-10 py-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold transition-all hover:shadow-lg hover:shadow-emerald-500/25">
